...
def manage_invoices():
    ...
    if request.method == 'GET':
        ...
        try:
            ...
            log_action(current_user_id, 'GET_INVOICES_SUCCESS', 'Retrieved all invoices')
        ...
        if not data or not data.get('items') or not data.get('total_amount'):
            log_action(current_user_id, 'CREATE_INVOICE_INVALID', 'Missing required fields for invoice creation')
        ...
        try:
            ...
            log_action(current_user_id, 'CREATE_INVOICE_SUCCESS', f'Created invoice {new_invoice.id}')
...
def manage_single_invoice(invoice_id):
    ...
    try:
        ...
        if not invoice:
            log_action(current_user_id, 'INVOICE_NOT_FOUND', f'Invoice {invoice_id} not found')
            return jsonify({'message': 'Invoice not found'}), 404

        if invoice.user_id != current_user_id and not (current_user and current_user.is_admin):
            log_action(current_user_id, 'INVOICE_UNAUTHORIZED', f'Unauthorized access for invoice {invoice_id}')
        ...
        if request.method == 'GET':
            ...
            log_action(current_user_id, 'GET_INVOICE_SUCCESS', f'Retrieved invoice {invoice_id}')
            return jsonify({
                'invoice_id': invoice.id,
                'user_id': invoice.user_id,
                'items': items,
            ...
            data = request.get_json()
            if not data:
                log_action(current_user_id, 'UPDATE_INVOICE_INVALID', 'No data provided for invoice update')
            ...
            db.session.commit()
            log_action(current_user_id, 'UPDATE_INVOICE_SUCCESS', f'Updated invoice {invoice_id}')
            ...
            db.session.commit()
            log_action(current_user_id, 'DELETE_INVOICE_SUCCESS', f'Deleted invoice {invoice_id}')
